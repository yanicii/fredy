...
class FredyPipelineExecutioner {
  ...
  _filterAfterDetails(listings) {
    // The spec filter already ran, but on what the search card knew. A card that names no room
    // count (Groth & Schneider's, for one) passes a minimum-rooms filter for want of a number, and
    // only the detail page says the flat has one room. The same three comparisons run once more
    // here, on the enriched facts; a listing the card fully described passes exactly as before.
    const withinSpecs = this._filterBySpecs(listings);
    ...
    const kept = withinSpecs.filter(this._providerConfig.filter);
    const removed = withinSpecs.length - kept.length;
    if (removed > 0) {
      logger.debug(
